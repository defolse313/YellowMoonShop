...
public class DetailsDisplayController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            String cakeId = null;
            if(request.getParameter("cake") == null) {
                cakeId = (String)request.getAttribute("CAKEID");
            } else {
                cakeId = request.getParameter("cake");
            }
            System.out.println("cakeId: " + cakeId);
            ShoppingBLO blo = new ShoppingBLO();
            Cake cake = blo.searchById(cakeId);
            request.setAttribute("CAKE", cake);
            HttpSession session = request.getSession();
            session.setAttribute("CAKES", cake);
        } catch (Exception e) {
            log("Error at DDController: " + e.getMessage());
            e.printStackTrace();
        } finally {
